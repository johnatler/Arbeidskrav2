...
```{r}
ggplot() + geom_point(data = heightsPosInc, mapping = aes(x = height, y = income), col = "darkseagreen4") +
  geom_smooth(data = heightsPosInc, mapping = aes(x = height, y = income), colour = "brown") + 
  theme_linedraw()
```
...
```{r}
ggplot(data = heightsPosInc) + 
  geom_bar(mapping = aes(x = heightInt, fill = sex))
```
